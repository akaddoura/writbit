...
{
   using namespace std;
   using QWidgetListMimeData = cw::QWidgetListMimeData;
   ...
   vector<NoteSheet*> QWidgetListWidget::getItems(bool onlySelected) const
   {
      vector<NoteSheet*> widgets;

      const int c = _layout->count();
      for (int i = 0; i < c; ++i)
         if (auto w = dynamic_cast<NoteSheet*>(_layout->itemAt(i)->widget()))
   ...
   std::vector<NoteSheet*> QWidgetListWidget::getSelectedItems() const
   {
   ...
   void QWidgetListWidget::mousePressEvent(QMouseEvent* event)
   {
      ...
      //mimeData->setData(mimeData->MimeType, QByteArray());
